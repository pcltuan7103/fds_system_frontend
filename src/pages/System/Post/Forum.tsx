...
const PostForumPage = () => {
    ...
    const approvedPost = sortedPosts.filter(
        (post) => post.status === "Approved"
    );

    const personalPost = sortedPosts.filter(
        (post) => post.posterId === userLogin?.accountId
    );
    const personalApprovedPost = personalPost.filter(
        (post) => post.status === "Approved"
    );
    const personalRejectedPost = personalPost.filter(
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            dispatch(
                getProfileApiThunk(String(userProfile?.accountId))
            ).unwrap(),
    ...
    const filteredApprovedPosts = selectedHashtag
        ? approvedPost.filter((post) => post.hashtags.includes(selectedHashtag))
        : approvedPost;
    ...
    return (
        <main id="post-forum">
                    ...
                    <div className="pfscc2">
                        <div className="pfscc2r1">
                            <div className="pf-tabs">
                                <div
                                    className={`pf-tabs-item ${
                                        activeTab === "noibat"
                                            ? "pf-tabs-item-actived"
                                            : ""
                                    }`}
            ...
            />
            <RemindLoginModal
                isOpen={isRemindLoginModalOpen}
                setIsOpen={setIsRemindLoginModalOpen}
            />
        </main>
    );
};
