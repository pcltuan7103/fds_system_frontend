...
const CreateRequestSupportModal: FC<CreateRequestSupportModalProps> = ({
    isOpen,
    setIsOpen,
}) => {
    ...
    const [lightboxCCCDIndex, setLightboxCCCDIndex] = useState<number | null>(
        null
    );
    const [imageCircumstancePreview, setImageCircumstancePreview] = useState<
        string[]
    >([]);
    const [lightboxCircumstanceIndex, setLightboxCircumstanceIndex] = useState<
        number | null
    >(null);
    ...
    const schema = Yup.object().shape({
        ...
        citizenIdImages: Yup.array()
            .of(Yup.string())
            .min(1, "Vui lòng tải lên ít nhất 1 ảnh CMND/CCCD"),
        ...
        requestedItems: Yup.array()
            .of(Yup.string())
            .min(1, "Vui lòng chọn ít nhất 1 mục cần hỗ trợ"),
        desiredQuantity: Yup.number()
            .min(20, "Số lượng cần hỗ trợ phải lớn hơn 0")
            .required("Số lượng cần hỗ trợ không được để trống"),
    });

    const onSubmit = async (
        values: RequestSupportActions,
        helpers: FormikHelpers<RequestSupportActions>
    ) => {
        try {
    ...
    const handleFileCircumstanceChange = async (
        e: React.ChangeEvent<HTMLInputElement>,
        setFieldValue: Function,
        setImagePreview: Function
    ) => {
                            ...
                            <Form onSubmit={handleSubmit} className="form">
                                ...
                                <div className="ccm-form-r1">
                                    ...
                                    <div className="form-100 form-field">
                                        ...
                                        <input
                                            ...
                                            className={classNames(
                                                ...
                                                {
                                                    "is-error":
                                                        errors.citizenIdImages &&
                                                        touched.citizenIdImages,
                                                }
                                            )}
                                        />
                                        {errors.citizenIdImages &&
                                    ...
                                    {lightboxCCCDIndex !== null && (
                                        <Lightbox
                                            images={imageCCCDPreview.map(
                                                (src) => ({ url: src })
                                            )}
                                            startIndex={lightboxCCCDIndex}
                                            onClose={() =>
                                                setLightboxCCCDIndex(null)
                                            }
                                        />
                                    )}
                                </div>
                                ...
                                <div className="ccm-form-r1">
                                    <div className="form-50 form-field">
                                        ...
                                        <Field
                                            name="phoneNumber"
                                            type="text"
                                            placeholder="Hãy nhập loại quà tặng"
                                            className={classNames(
                                                "form-input",
                                                {
                                                    "is-error":
                                ...
                                <div className="ccm-form-r3">
                                    ...
                                    <div className="form-50 form-field">
                                        <label className="form-label">
                                            Số người mong muốn hỗ trợ
                                            <span>*</span>
                                        </label>
                                        <Field
                                            name="householdSize"
                                            type="number"
                                            placeholder="Hãy nhập số người trong hộ gia đình"
                                            className={classNames(
                                                "form-input",
                                                {
                                                    "is-error":
                                    ...
                                    <div className="form-50 form-field">
                                        ...
                                        {errors.specialMembers &&
                                            touched.specialMembers && (
                                                <span className="text-error">
                                                    {errors.specialMembers}
                                                </span>
                                            )}
                                    ...
                                    <div className="form-100 form-field">
                                        ...
                                        <input
                                            ...
                                            multiple
                                            onChange={(e) =>
                                                handleFileCircumstanceChange(
                                                    e,
                                                    setFieldValue,
                                                    setImageCircumstancePreview
                                                )
                                            }
                                            className={classNames(
                                                "form-input",
                                                {
                                                    "is-error":
                                                        errors.circumstanceImages &&
                                                        touched.circumstanceImages,
                                                }
                                            )}
                                        />
                                        {errors.circumstanceImages &&
                                    ...
                                    {lightboxCircumstanceIndex !== null && (
                                        <Lightbox
                                            images={imageCircumstancePreview.map(
                                                (src) => ({ url: src })
                                            )}
                                            startIndex={
                                                lightboxCircumstanceIndex
                                            }
                                            onClose={() =>
                                                setLightboxCircumstanceIndex(
                                                    null
                                ...
                                <div className="ccm-form-r3">
                                    <div className="form-100 form-field">
                                        <label className="form-label">
                                            Chọn phần quà bạn cần hỗ trợ
                                            <span>*</span>
                                        </label>

                                        {/* Hiển thị các checkbox: cả mặc định và người dùng nhập thêm */}
                                        <Field name="requestedItems">
                                            {({ field }: any) => {
                                                const { value } = field;
                                                const predefinedItems = [
                                                    ...
                                                    "Mì tôm",
                                                    "Nước tương",
                                                    "Sữa cho trẻ em hoặc người già",
                                                    "Rau củ quả",
                                                    "Thực phẩm đóng hộp",
                                                ];

                                                // Lọc ra những phần quà do người dùng thêm
                                                const customItems =
                                                    value.filter(
                                                        (v: string) =>
                                                            !predefinedItems.includes(
                                                                v
                                                            )
                                                    );
                                                const allItems = [
